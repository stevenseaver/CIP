                                ...
                                <?php foreach ($materialStock as $inv) :
                                    if ($before != $inv['transaction_id']) { ?>
                                        <tr>
                                            ...
                                            <?php }; ?>
                                            <td>
                                                <a href="<?= base_url('production/roll_details/') . $inv['transaction_id'] ?>" class="btn btn-primary btn-sm rounded mb-1"><i class="bi bi-info-circle-fill"> </i>Details</a>
                                                <a href="<?= base_url('production/add_roll/') . $inv['transaction_id'] ?>" class="btn btn-warning btn-sm rounded mb-1"><i class="bi bi-pencil-fill"> </i>Input Roll</a>
                                                <!-- <a href="<?= base_url('production/rollToGBJ/') . $inv['transaction_id']?>" class="badge badge-success">Transfer to GBJ</a> -->
                                                <?php if ($inv['transaction_status'] == 1){ ?>
                                                    <a data-toggle="modal" data-target="#deleteRollModal" data-po="<?= $inv['transaction_id']  ?>" class="btn btn-danger btn-sm rounded mb-1"><i class="bi bi-trash-fill"> </i>Delete Roll Input</a>
                                                <?php } else { ?>
                                                    <a href="" class="btn btn-secondary btn-sm rounded mb-1" disabled><i class="bi bi-trash-fill"> </i>Delete Roll Input</a>
                                                <?php }; ?>
                                            </td>
                                        </tr>
                                    <?php

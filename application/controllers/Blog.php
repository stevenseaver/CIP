...
class Blog extends CI_Controller
{
    ...
    public function add_new_post()
    {
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('employee/add-new-post', $data);
        $this->load->view('templates/footer');
    }
    ...
    {
        //load user data per session
        $data['title'] = 'Edit Blog Posts';
        $data['user'] = $this->db->get_where('user', ['nik' =>
        $this->session->userdata('nik')])->row_array();
        //get blog based on id
        $data['blog_content'] = $this->db->get_where('blogpost', ['id' => $id])->result_array();
        $data['post_type'] = $this->db->get('blog_type')->result_array();

        // var_dump($data['blog_content']);

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('employee/edit-blog-post', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function edit_post($id)
    {
        ...
        $this->form_validation->set_rules('meta', 'meta title', 'trim|required');
        $this->form_validation->set_rules('summary', 'summary', 'trim|required');
        $this->form_validation->set_rules('blog_content', 'blog_content', 'trim|required');
